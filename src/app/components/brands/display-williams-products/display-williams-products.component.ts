...
@Component({
  selector: 'app-display-williams-products',
  templateUrl: './display-williams-products.component.html',
  styleUrls: ['./display-williams-products.component.css']
})
export class DisplayWilliamsProductsComponent implements OnInit {

  allProducts: Product[] = [];

  constructor(private productService: ProductService, private router: Router, private titleService: Title) { 
    this.titleService.setTitle("Williams Products - Soul Sounds")
  }

  ngOnInit(): void {
    this.productService.getProductsByBrand("Williams").subscribe(
