...
@Component({
  selector: 'app-display-epiphone-products',
  templateUrl: './display-epiphone-products.component.html',
  styleUrls: ['./display-epiphone-products.component.css']
})
export class DisplayEpiphoneProductsComponent implements OnInit {

  allProducts: Product[] = [];

  constructor(private productService: ProductService, private router: Router, private titleService: Title) { 
    this.titleService.setTitle("Epiphone Products - Soul Sounds")
  }

  ngOnInit(): void {
    this.productService.getProductsByBrand("Epiphone").subscribe(
