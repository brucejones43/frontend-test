...
@Component({
  selector: 'app-display-yamaha-products',
  templateUrl: './display-yamaha-products.component.html',
  styleUrls: ['./display-yamaha-products.component.css']
})
export class DisplayYamahaProductsComponent implements OnInit {

  allProducts: Product[] = [];

  constructor(private productService: ProductService, private router: Router, private titleService: Title) { 
    this.titleService.setTitle("Yamaha Products - Soul Sounds")
  }

  ngOnInit(): void {
    this.productService.getProductsByBrand("Yamaha").subscribe(
