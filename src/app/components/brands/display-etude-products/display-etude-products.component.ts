...
@Component({
  selector: 'app-display-etude-products',
  templateUrl: './display-etude-products.component.html',
  styleUrls: ['./display-etude-products.component.css']
})
export class DisplayEtudeProductsComponent implements OnInit {

  allProducts: Product[] = [];

  constructor(private productService: ProductService, private router: Router, private titleService: Title) { 
    this.titleService.setTitle("Etude Products - Soul Sounds")
  }

  ngOnInit(): void {
    this.productService.getProductsByBrand("Etude").subscribe(
