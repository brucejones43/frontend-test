...
@Component({
  selector: 'app-display-numark-products',
  templateUrl: './display-numark-products.component.html',
  styleUrls: ['./display-numark-products.component.css']
})
export class DisplayNumarkProductsComponent implements OnInit {

  allProducts: Product[] = [];

  constructor(private productService: ProductService, private router: Router, private titleService: Title) { 
    this.titleService.setTitle("Numark Products - Soul Sounds")
  }

  ngOnInit(): void {
    this.productService.getProductsByBrand("Numark").subscribe(
