...
@Component({
  selector: 'app-display-martin-products',
  templateUrl: './display-martin-products.component.html',
  styleUrls: ['./display-martin-products.component.css']
})
export class DisplayMartinProductsComponent implements OnInit {

  allProducts: Product[] = [];

  constructor(private productService: ProductService, private router: Router, private titleService: Title) { 
    this.titleService.setTitle("Martin Products - Soul Sounds")
  }

  ngOnInit(): void {
    this.productService.getProductsByBrand("Martin").subscribe(
