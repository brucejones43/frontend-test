...
@Component({
  selector: 'app-display-taylor-products',
  templateUrl: './display-taylor-products.component.html',
  styleUrls: ['./display-taylor-products.component.css']
})
export class DisplayTaylorProductsComponent implements OnInit {

  allProducts: Product[] = [];

  constructor(private productService: ProductService, private router: Router, private titleService: Title) { 
    this.titleService.setTitle("Taylor Products - Soul Sounds")
  }

  ngOnInit(): void {
    this.productService.getProductsByBrand("Taylor").subscribe(
