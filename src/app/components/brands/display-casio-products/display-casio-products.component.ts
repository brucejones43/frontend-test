...
@Component({
  selector: 'app-display-casio-products',
  templateUrl: './display-casio-products.component.html',
  styleUrls: ['./display-casio-products.component.css']
})
export class DisplayCasioProductsComponent implements OnInit {

  allProducts: Product[] = [];

  constructor(private productService: ProductService, private router: Router, private titleService: Title) { 
    this.titleService.setTitle("Casio Products - Soul Sounds")
  }

  ngOnInit(): void {
    this.productService.getProductsByBrand("Casio").subscribe(
