...
@Component({
  selector: 'app-display-drum-products',
  templateUrl: './display-drum-products.component.html',
  styleUrls: ['./display-drum-products.component.css']
})
export class DisplayDrumProductsComponent implements OnInit {

  allProducts: Product[] = [];

  constructor(private productService: ProductService, private router: Router, private titleService: Title) { 
    this.titleService.setTitle("Drumsets - Soul Sounds")
  }

  ngOnInit(): void {
    this.productService.getProductsByCategory("Drums").subscribe(
